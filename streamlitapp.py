import os
os.environ["PANDAS_USE_PYARROW"] = "0"   # 🔥 disable Arrow completely
...
@st.cache_data
def load_resources():
    df = pd.read_csv('df_final.csv', engine='python')  # safer read
...
if page == "Insights & Model Validation":
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["Demand Patterns", "Environmental Impact", "Model Validation", "Correlation Analysis"]
    )
    ...
    with tab1:
        ...
        hourly_demand['Day Type'] = hourly_demand['workingday_Working Day'].apply(
            lambda x: 'Working Day' if x == 1 else 'No work'
        )
    ...
    with tab2:
        ...
        if 'weathersit_Light Snow' in df_plot_weather.columns:
    ...
    with tab4:
        ...
        # 🔥 FULL SAFE PIPELINE (NO ARROW, NO ERRORS)
        df_corr_data = df_final.select_dtypes(include=[np.number]).copy()

        # Convert fully to numpy → then back to pandas (kills Arrow completely)
        df_corr_data = pd.DataFrame(
            df_corr_data.to_numpy(dtype=float),
